...
def build_libpng():
	...
	os.chdir(png_dir)
	env = os.environ.copy()
	env['CFLAGS'] = '-L../zlib/lib -I../zlib/include -mmacosx-version-min=' + min_osx_version
	env['CPPFLAGS'] = '-L../zlib/lib -I../zlib/include -mmacosx-version-min=' + min_osx_version
...
def build_sdl():
	...
	check_call(['../configure', '--prefix=' + prefix,
		...
		'--disable-render', '--disable-haptic',
		#'--disable-file', '--disable-filesystem', #something wrong with linking when these are disabled -- not properly dummied?
		'--disable-loadso',
		'--disable-power',
		'--enable-sse2',
	...
	with open(sdl_out + "/bin/sdl2-config", 'r') as f:
		...
		for line in f:
			if re.match('^prefix=.*$', line) != None:
				...
				out.append("prefix=kit-libs-osx/" + sdl_out + "\n")
				found_prefix = True
			else:
				out.append(line)
		assert(found_prefix)
